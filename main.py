...
def H_C(edge, gamma):
    return 0.5 * (1 - qml.PauliZ(edge[0]) @ qml.PauliZ(edge[1]))

def Mixer_C(edge, beta):
    return qml.PauliX(edge[0]) + qml.PauliX(edge[1])

@qml.qnode(dev)
def circuit(gammas, betas, edges=None, n_layers=1):

    for wires in range(n_wires):
...
qml.draw_mpl(circuit, style="pennylane")(params[0], params[1], edges=graph)
